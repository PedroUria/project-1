...
Visually, the Contract and Churn variables don't seem to be independent, counter to what our hypothesis suggests, but we'll revisit this again with a more rigorous analysis later on. Based on this, Churn may be a significantly dependent on Contract Type.
...
No need to do any further testing here, this is clearly not a normally distributed variable.
...
This histogram of tenure for customers that have churned, reinforces the trend we saw when looking at Churn earlier. It appears that more of the Churning behavior is occuring in the most recently added customers.  
...
We can clearly see that of the customers that started with the company during the month the data was recorded, the majority of them have already Churned.  
...
## Updated Null Hypothesess
## Updated Alternative Hypotheses
...
# who didn't churn, 
str(telco)
t.test(chrn_yes$tenure,chrn_no$tenure,alternative = "two.sided",var.equal = FALSE)
...
yes_sam = sample(chrn_yes$customerID,size=1000)
no_sam = sample(chrn_no$customerID,size=1000)
yess = chrn_yes[which(chrn_yes$customerID%in%yes_sam),]
dim(yess)
nos = chrn_no[which(chrn_no$customerID%in%no_sam),]
...
t.test(chrn_yes$MonthlyCharges,chrn_no$MonthlyCharges,alternative = "two.sided",var.equal = FALSE)
dim(nos)
