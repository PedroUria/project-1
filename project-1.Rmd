...
3. There is a significant difference in the mean monthly charges for Customers who decided to Churn, and those who did no decide to Churn. $\mu_{Churned}-\mu_{Not Churned}\neq0$
...
```{r}
# Check the percentage of churned customers who started their subscriptions within the last 10 months in the dataset. 
...
```{r}
count(telco %>% filter(OnlineSecurity == "No", Churn == "Yes"))/count(telco %>% filter(Churn == "Yes",OnlineSecurity !="No internet service"))
...
```{r}
count(telco %>% filter(TechSupport == "No", Churn == "Yes"))/count(telco %>% filter(Churn == "Yes", TechSupport != "No internet service"))
...
```{r}
count(telco %>% filter(OnlineBackup == "No", Churn == "Yes"))/count(telco %>% filter(Churn == "Yes", OnlineBackup != "No internet service"))
...
```{r}
count(telco %>% filter(DeviceProtection == "No", Churn == "Yes"))/count(telco %>% filter(Churn == "Yes", DeviceProtection != "No internet service"))
...
Conduct our t-test for tenure. 
```{r}
t.test(telco_yes$MonthlyCharges,telco_no$MonthlyCharges,alternative = "greater",var.equal = FALSE)
```
...
More specifically, we found that Churn is not independent of contract type, the sample mean tenure of customers who churned is less than the sample mean tenure of customers who did not churn and the sampe mean monthly charges incurred by the customers who churned is higher than the sample mean monthly charges incurred by the customers who churned. 

This means that customers with different levels commitment to a company have different probabilties of churning, customers who have longer tenures with a company are less likely to churn, on average, and customers who have larger bills are more likely to churn, on average. 
