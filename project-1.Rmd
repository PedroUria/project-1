...
```{r}
count(telco %>% filter(OnlineSecurity == "No", Churn == "Yes"))/count(telco %>% filter(Churn == "Yes"))
```

As we can see, ~78% of the customers that churned did not have OnlineSecurity.
...
```{r}
count(telco %>% filter(TechSupport == "No", Churn == "Yes"))/count(telco %>% filter(Churn == "Yes"))
```

As we can see ~77% of the customers that churned this month, had opted out of the company's tech support services.
...
```{r}
count(telco %>% filter(OnlineBackup == "No", Churn == "Yes"))/count(telco %>% filter(Churn == "Yes"))
```

As we can see ~66% of the customers that churned this month, had opted out of the company's online backup services.
...
```{r}
count(telco %>% filter(PaperlessBilling == "Yes", Churn == "Yes"))/count(telco %>% filter(Churn == "Yes"))
```
...
```{r}
hist(telco$MonthlyCharges, main = "Monthly Charges Histogram", xlab = "Monthly Charges ($)", ylab = "Count")
...
```{r}
count(telco %>% filter(DeviceProtection == "No", Churn == "Yes"))/count(telco %>% filter(Churn == "Yes"))
```

As we can see ~65% of the customers that churned this month, had opted out of the company's device protection plan.
...
```{r}
count(telco %>% filter(Dependents == "No", Churn == "Yes"))/count(telco %>% filter(Churn == "Yes"))
```
...
```{r}
count(telco %>% filter(Partner == "No", Churn == "Yes"))/count(telco %>% filter(Churn == "Yes"))
```

As we can see ~64% of the customers that churned this month, had opted into of the company's paperless billing option.
...
```{r}
count(telco %>% filter(SeniorCitizen == "No", Churn == "Yes"))/count(telco %>% filter(Churn == "Yes"))
```
...
```{r}
count(telco %>% filter(gender == "Male", Churn == "Yes"))/count(telco %>% filter(Churn == "Yes"))
```
...
```{r}
count(telco %>% filter(PhoneService == "Yes", Churn == "Yes"))/count(telco %>% filter(Churn == "Yes"))
```
...
```{r}
count(telco %>% filter(InternetService == "Fiber optic", Churn == "Yes"))/count(telco %>% filter(Churn == "Yes"))
```
...
```{r}
count(telco %>% filter(StreamingTV == "No internet service", Churn == "Yes"))/count(telco %>% filter(StreamingTV == "No internet service"))
```
...
```{r}
##Modeling 
cten_box<-ggplot(telco, aes(x=Churn, y=tenure, color=Churn)) +geom_boxplot()
cten_box
# the median tenure of the customers who churned is roughly 1/2 the tenure of customers who didn't churn.
mean(telco_yes$tenure)
mean(telco_no$tenure)
# The same can be said for the means, which is why we 
# Want to conduct a t-test to determine if these means are different.

# Check assumptions before conducting our t-test. 

#1: Independence 

# Although we are not sure if our Telco data is synthetically generated or from real customers who might communicate with each other about their service, we still think that it is reasonable to assume that customers  are churning independently of each other.  
...
hist(telco$tenure, ylab = "count", xlab = "Tenure (months)", main = "Tenure Histogram")# not normal
hist(telco_no$tenure) # not normal 
hist(telco_yes$tenure)# not normal and definitely skewed right, 
# but our sample contains 1869 observations, so we think we will 
# still be able to use a t-test. 

qqnorm(telco$tenure)
qqline(telco$tenure)

# Note, T-tests are robust to non-normal data if their sample sizes are high enough (>30) because of the central limit theorem.

## Tenure doesn't appear to be normally distrubted but since the sample size of each grouping of tenure, n1 (Churn = Yes) and n2 (Churn = no), are large, (n1 = 1869 &  n2 5174, respectively) is much higher than 30 and the overall skewness of tenure (.2394377) is .significantly less than 1, we beleive that we should be able to use a t-test to determine if there is a significant difference between the mean tenures of customers who did and did not churn. 
...
skewness(telco_no$MonthlyCharges) # skewed slighly less left
skewness(telco_yes$MonthlyCharges) # fairly skewed to the left, but abs (-.7282035) is < 1. 

qqnorm(telco$MonthlyCharges) 
qqline(telco$MonthlyCharges)# again, the data is definitely not normal. 
...
# Customers who churned appear to have higher monthly charges, but more interestingly, the IQR of the boxplot for the customers who churned is much tighter, meaning the middle 50% of the customers who churned are definitely paying more than the customers who did not churn. 

sd(telco_no$MonthlyCharges)
sd(telco_yes$MonthlyCharges)
# sd's are different. 
leveneTest(MonthlyCharges~Churn,data=telco)
# Reject H0, so our group varainces are definitely different, use Welch's test. 


## MonthlyCharges doesn't appear to be normally distrubted but since the sample size of each grouping of MonghlyCharges, n1 (Churn = Yes) and n2 (Churn = no), are large, (n1 = 1869 &  n2 5174, respectively) is much higher than 30 and the overall skewness of tenure (-.2204305) is .significantly less than 1, we beleive that we should be able to use a t-test to determine if there is a significant difference between the mean tenures of customers who did and did not churn. 
